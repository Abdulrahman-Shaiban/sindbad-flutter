...
class SupplierProductsStateNotifier
    extends StateNotifier<SupplierProductsState> {
  SupplierProductsStateNotifier() : super(const SupplierProductsInitial());

  Future<void> getSupplierProducts() async {
    try {
      state = const SupplierProductsLoading();
      // final supplierProductsResponse = await _supplierRepository.getSupplierProducts(supplier: supplier);
      final supplierProductsResponse = await getFakeSupplierProducts();
      state = SupplierProductsLoaded(products: supplierProductsResponse);
    } catch (e) {
      state = const SupplierProductsError(error: 'Error');
    }
  }
}

getFakeSupplierProducts() {
  return [
    Product(
      id: '1',
